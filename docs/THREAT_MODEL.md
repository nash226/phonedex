...
| Notification actions expose a durable token | Anyone with notification metadata could reply as the user | Native notification metadata contains routing context only; the iOS app loads its credential from Keychain when the action is handled. Pushcut fallback actions carry only a ten-minute, single-use opaque grant whose hash is consumed transactionally and bound to the task version and idempotent command. `npm run test:security` covers credential-free payloads and replay rejection. Legacy bootstrap and query-token compatibility remains a separately documented migration path. |
| Query or form-body credentials are copied into history or analytics | Credential replay or accidental support disclosure | Native bridge and configuration URLs reject credential-bearing endpoint URLs; privacy administration requires the `Authorization` header. Legacy `/reply` and bootstrap token compatibility are explicit, disabled-by-default migration paths, and `NODE_ENV=production` or `PHONEDEX_PRODUCTION=true` rejects them at startup. |
