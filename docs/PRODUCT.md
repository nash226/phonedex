...
| Review changes | Mobile diff summary, file list, patch detail, and validation results | **Current, partial.** Supported agents can export bounded patches for native mobile review alongside a native file-level summary, structured validation results, artifact metadata, and source references; full artifact downloads remain target work. |
