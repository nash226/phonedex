...
const { evaluateReleaseMatrix, DEFAULT_MAX_AGE_DAYS } = require("../lib/phonedex-release-matrix");

const flags = parseFlags(process.argv.slice(2));
if (!flags.input) {
  console.error("Usage: node scripts/release-matrix.js --input <evidence.json> [--now <ISO-8601>] [--max-age-days <n>]");
  process.exitCode = 2;
} else {
  try {
    ...
    const report = evaluateReleaseMatrix(input, { now, maxAgeDays });
    process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
    if (!report.ok) process.exitCode = 1;
  } catch (error) {
    console.error(`Release matrix preflight failed: ${error.message}`);
    process.exitCode = 2;
  }
}
