...
Or create a short-lived invite link that opens the same setup page without
putting the main hub token in the URL:
...
started, reached self-test, or failed during bootstrap. The hub also sends
PhoneDex alerts when a target bootstrap starts, fails, or completes. PhoneDex
keeps only the newest few active invite links by default so older unused setup
URLs do not compete with the current one.
...
On an agent device, run a direct forwarding proof with:
...
The hub service also sends a throttled PhoneDex coverage alert when expected
devices are missing or stale. Send one manually with:

```sh
npm run devices:alert
```
...
| `npm run devices:alert` | Send a PhoneDex alert when expected devices are missing or stale. |
...
| `PHONEDEX_COVERAGE_ALERTS` | No | Hub-only missing-device alerts. Defaults to `true`; set to `false` to disable. |
| `PHONEDEX_COVERAGE_ALERT_INTERVAL_MS` | No | Minimum time between repeated coverage alerts for the same missing/stale device set. Defaults to six hours. |
| `PHONEDEX_AGENT_INVITE_TTL_MS` | No | How long `npm run agent:invite` links remain valid. Defaults to 24 hours. |
| `PHONEDEX_AGENT_INVITE_MAX_ACTIVE` | No | Maximum active setup invite links to keep. Defaults to 5, pruning the oldest unused links first. |
| `PHONEDEX_RETENTION_DAYS` | No | Apply a bounded task/activity retention window when the hub starts. `0` (the default) keeps history until the user explicitly changes policy. Valid range is 0–3650 days. |
...
- `data/devices.json` stores the latest service heartbeat for each configured
  device.
- `data/privacy-policy.json` stores the bounded retention policy.
- `data/privacy-audit.jsonl` stores content-free retention and history-deletion audits.
...
`GET /privacy` reports bounded counts,
`GET /privacy/export` returns a redacted export, `POST /privacy/retention`
requires `APPLY_PHONEDEX_RETENTION` plus a day count, and `POST /privacy/delete`
requires the exact `DELETE_PHONEDEX_HISTORY` confirmation. Deletion removes
task and activity history while retaining current device inventory; it is never
triggered by a normal sync or app refresh.
