...
function createPhoneDexLifecycleManager({ cfg, recordTask, updateTask, findTask, appendEvent }) {
  ...
  return {
    capabilities() {
      return {
        ...
        approval: supportsAdapterCapability(cfg.adapter, "approval.respond") &&
          typeof cfg.approvalExecutor === "function",
        handoff: supportsAdapterCapability(cfg.adapter, "desktop.handoff")
      };
    },
    ...
    async execute(command) {
      ...
      if (!["create_task", "cancel", "retry", "approve", "reject", "handoff"].includes(kind)) {
        throw lifecycleError("unsupported_command", "This lifecycle command is not supported.", 422);
      }

      const capability = ["approve", "reject"].includes(kind)
        ? "approval"
        : kind === "create_task" ? "create" : kind;
      if (!this.capabilities()[capability]) {
        throw lifecycleError(
          "capability_unsupported",
          `The selected agent does not support ${kind === "create_task" ? "task creation" : kind === "handoff" ? "desktop handoff" : `task ${kind}ing`}.`,
          409
        );
      }
      ...
      if (kind === "approve" || kind === "reject") return respondToApproval(command);
      if (kind === "handoff") return prepareDesktopHandoff(command);
  ...
  async function respondToApproval(command) {
    const task = requireTask(command);
    const result = await cfg.approvalExecutor({ command, task });
    if (!result || typeof result !== "object") {
      throw lifecycleError(
        "approval_executor_invalid",
        "The originating agent returned no approval result.",
        502
      );
    }
    return result;
  ...
  async function prepareDesktopHandoff(command) {
    const task = requireTask(command);
    ...
    appendEvent({ type: "desktop-handoff-prepared", taskId: task.id });
    return {
      task,
      handoff,
      state: "completed",
      message: `Desktop handoff prepared for ${handoff.machineName}.`
    };
  }
