...
function createPhoneDexPrivacy(dataDir) {
  ...
  return {
    ...
    applyRetention(retentionDays, options = {}) {
      ...
      if (days > 0) {
        for (const task of store.listTasks()) {
          if (isBefore(task, cutoff)) {
            if (store.removeTask(task.id).removed) deletedTaskCount += 1;
          }
        }
      ...
      return { policy, deletedTaskCount, deletedActivityCount };
    },
    ...
    deleteHistory(options = {}) {
      ...
      const result = { deletedTaskCount, deletedActivityCount };
...
function isSensitiveKey(key) {
  return /token|secret|password|api[_-]?key|authorization|cookie|credential|hookpayload|rawhook|replyurl|publicurl|codexhome|hostname|pid/i.test(key);
}
...
function redactSensitiveText(value) {
  return String(value)
    ...
    .replace(/\b(password|token|secret|api[_ -]?key)\b\s*:?\s*([^\s`"'<>]{8,})/gi, "$1: [redacted]");
}
