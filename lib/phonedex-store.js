...
const { rotateIdentityCredential } = require("./phonedex-identity");
  ...
  return {
    ...
    updateTask(taskId, changes = {}) {
      return transaction((state) => {
        ...
        if (index < 0) return { changed: false, found: false, task: null };

        const current = state.tasks[index];
        const next = addTaskProtocolFields({
          ...current,
          ...changes,
          id: current.id,
          createdAt: current.createdAt || current.at,
          updatedAt: changes.updatedAt || new Date().toISOString(),
          version: Number.isInteger(changes.version) ? changes.version : (current.version || 1) + 1
        });
        state.tasks[index] = next;
        recordChange(state, "task", next.id, next, false);
        return { changed: true, found: true, task: next };
      });
    },
    ...
    rotateIdentity({ identityId, deviceId, now = new Date().toISOString() } = {}) {
      return transaction((state) => {
        const index = state.identities.findIndex((identity) =>
          (identityId && identity.id === identityId) ||
          (!identityId && deviceId && identity.deviceId === deviceId)
        ...
        const current = state.identities[index];
        if (current.status === "revoked") {
          return { changed: false, found: true, code: "identity_revoked", identity: current };
        }

        const rotated = rotateIdentityCredential(current, new Date(now));
        state.identities[index] = rotated.identity;
        return {
          changed: true,
          found: true,
    ...
    createNotificationActionGrant(grant) {
      return transaction((state) => {
        const now = Date.now();
        state.notificationActionGrants = state.notificationActionGrants.filter(
          (candidate) => Date.parse(candidate.expiresAt || "") > now && !candidate.usedAt
        );
        state.notificationActionGrants.push(grant);
        return grant;
      });
    },

    consumeNotificationActionGrant({ grantHash, now = new Date().toISOString() } = {}) {
      return transaction((state) => {
        const index = state.notificationActionGrants.findIndex(
          (grant) => grant.grantHash === grantHash
        );
        if (index < 0) return { changed: false, ok: false, code: "action_invalid" };

        const grant = state.notificationActionGrants[index];
        if (grant.usedAt) {
          return { changed: false, ok: false, code: "action_used" };
        }
        if (Date.parse(grant.expiresAt || "") <= Date.parse(now || "")) {
          return { changed: false, ok: false, code: "action_expired" };
        }

        state.notificationActionGrants[index] = { ...grant, usedAt: now };
        return { ok: true, grant: state.notificationActionGrants[index] };
      });
    },
    ...
    clearTaskHistory() {
      const result = transaction((state) => {
        ...
        state.events = [];
        state.changes = [];
        state.notificationActionGrants = [];
...
function migrateState(value) {
  ...
  const state = {
    ...
    notificationActionGrants: Array.isArray(value.notificationActionGrants)
      ? value.notificationActionGrants
      : [],
...
function validateState(state) {
  ...
  if (!Array.isArray(state.tasks) || !Array.isArray(state.devices) || !Array.isArray(state.events) ||
      ...
      !Array.isArray(state.notificationActionGrants) ||
      !Array.isArray(state.changes)) {
    throw new Error("PhoneDex store tasks, devices, events, identities, pairing grants, notification action grants, and changes must be arrays");
  }
  ...
  for (const [index, record] of state.notificationActionGrants.entries()) {
    if (!record || typeof record !== "object" || Array.isArray(record)) {
      throw new Error(`PhoneDex notification action grant at index ${index} must be an object`);
    }
  }
