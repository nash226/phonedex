...
function createPhoneDexStore(dataDir) {
  ...
  return {
      ...
      return transaction((state) => {
        ...
        if (duplicateIndex >= 0) {
          ...
          if (merged && JSON.stringify(merged) !== JSON.stringify(duplicate)) {
            ...
            return { created: false, merged: true, task: merged };
          }
          return { changed: false, created: false, merged: false, task: duplicate };
        }
    ...
    clearTaskHistory() {
      const result = transaction((state) => {
        ...
        state.tasks = [];
        state.changes = [];
        return { deletedTaskCount, deletedChangeCount };
      }, { createBackup: false });
    ...
    readSync(options = {}) {
      ...
      if (!cursor) {
        return buildSnapshotPage(
          state,
          { mode: "snapshot", position: latestPosition, revision: state.revision, taskOffset: 0, deviceOffset: 0 },
          limit,
          latestPosition
        );
      }
  ...
  function buildSnapshotPage(state, cursor, limit, latestPosition) {
    ...
    const complete = taskOffset >= tasks.length && deviceOffset >= devices.length;
...
function validateState(state) {
  ...
  if (!Array.isArray(state.tasks) || !Array.isArray(state.devices) ||
      !Array.isArray(state.identities) || !Array.isArray(state.pairingGrants) ||
      !Array.isArray(state.changes)) {
    throw new Error("PhoneDex store tasks, devices, identities, pairing grants, and changes must be arrays");
  }
  ...
  for (const [index, change] of state.changes.entries()) {
    ...
    if (!["task", "device"].includes(change.kind)) {
