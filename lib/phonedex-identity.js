...
function rotateIdentityCredential(identity, now = new Date()) {
  if (!identity || typeof identity !== "object") {
    throw new Error("Cannot rotate a missing PhoneDex identity.");
  }
  const credential = crypto.randomBytes(32).toString("base64url");
  const issuedAt = now.toISOString();
  return {
    ...
    identity: {
      ...identity,
      credentialHash: hashSecret(credential),
      credentialVersion: (Number.isInteger(identity.credentialVersion)
...
function publicIdentity(identity) {
  ...
  return {
    ...
    ...(Number.isInteger(identity.credentialVersion)
      ? { credentialVersion: identity.credentialVersion }
      : {}),
    ...(identity.lastRotatedAt ? { lastRotatedAt: identity.lastRotatedAt } : {}),
