...
function pruneArtifacts(dataDir, options = {}) {
  const root = path.resolve(dataDir);
  const indexPath = path.join(root, ARTIFACT_INDEX);
  const index = readIndex(indexPath);
  const before = Number.isFinite(options.before) ? options.before : null;
  const taskIds = new Set(Array.isArray(options.taskIds) ? options.taskIds : []);
  let deletedCount = 0;
  let deletedBytes = 0;

  for (const [downloadId, record] of Object.entries(index)) {
    const storedAt = Date.parse(record?.storedAt || "");
    const expired = options.all === true || (before !== null && !Number.isNaN(storedAt) && storedAt < before);
    if (!expired && !taskIds.has(record?.taskId)) continue;

    const filePath = path.join(root, ARTIFACT_DIRECTORY, `${downloadId}.bin`);
    try { deletedBytes += fs.statSync(filePath).size; } catch { /* index may outlive its payload */ }
    try { fs.rmSync(filePath, { force: true }); } catch { continue; }
    delete index[downloadId];
    deletedCount += 1;
  }

  if (deletedCount > 0) writeJsonAtomically(indexPath, index);
