...
const REQUIRED_PLATFORMS = Object.freeze({
  performance: Object.freeze(["ios", "macos", "windows"]),
  battery: Object.freeze(["ios"]),
  accessibility: Object.freeze(["ios"]),
  localization: Object.freeze(["ios"]),
  crash: Object.freeze(["ios"])
});
...
function evaluateQualityGates(input, { now = new Date(), maxAgeDays = DEFAULT_MAX_AGE_DAYS } = {}) {
  ...
  if (!Array.isArray(records)) {
    ...
    for (const record of records.slice(0, MAX_RECORDS)) {
      ...
      const missingRequiredPlatforms = (REQUIRED_PLATFORMS[id] ?? []).filter((platform) => !platforms.includes(platform));
      if (missingRequiredPlatforms.length > 0) {
        recordIssues.push(`quality gate is missing required platform coverage: ${missingRequiredPlatforms.join(", ")}`);
      }
...
module.exports = { DEFAULT_MAX_AGE_DAYS, MAX_RECORDS, REQUIRED_GATES, REQUIRED_PLATFORMS, SCHEMA, SOURCE_REVISION_PATTERN, evaluateQualityGates };
