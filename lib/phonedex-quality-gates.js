...
function evaluateQualityGates(input, { now = new Date(), maxAgeDays = DEFAULT_MAX_AGE_DAYS } = {}) {
  ...
  if (!Array.isArray(records)) {
    ...
    for (const record of records.slice(0, MAX_RECORDS)) {
      ...
      const { platforms, invalidPlatforms } = normalizePlatforms(record?.platforms);
  ...
  return { schema: SCHEMA, generatedAt: now.toISOString(), maxAgeDays, requiredGates: REQUIRED_GATES, gates, ok: issues.length === 0, issues };
}

function normalizePlatforms(value) {
  if (!Array.isArray(value)) return { platforms: [], invalidPlatforms: [] };
  const platforms = [];
  ...
  return { platforms, invalidPlatforms };
}
...
module.exports = { DEFAULT_MAX_AGE_DAYS, MAX_RECORDS, REQUIRED_GATES, SCHEMA, evaluateQualityGates };
