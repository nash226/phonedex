...
function normalizeArtifact(value) {
  ...
  const sha256 = normalizeDigest(value.sha256 || value.digest);
  if (sha256) result.sha256 = sha256;
  const downloadId = normalizeDownloadId(value.downloadId);
  if (downloadId) result.downloadId = downloadId;
  const mediaType = boundedString(value.mediaType || value.contentType, MAX.mediaType).toLowerCase();
