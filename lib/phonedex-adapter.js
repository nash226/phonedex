...
function createCodexAdapter({
  ...
  appServerBin = "",
  workspaceRoots = [],
  allowExperimentalForeground = false
} = {}) {
  ...
  const experimentalForegroundEnabled = normalizedMode !== "foreground" || allowExperimentalForeground === true;
  const executableConfigured = normalizedMode === "foreground" || nonEmptyString(executable);
  const ready = platformSupported && modeSupported && experimentalForegroundEnabled && executableConfigured;
  ...
  if (normalizedMode === "foreground" && !experimentalForegroundEnabled) {
    limitations.push("Experimental foreground paste is disabled; set PHONEDEX_ENABLE_EXPERIMENTAL_FOREGROUND=true to opt in.");
  }
