...
function validateTask(value, errors) {
  ...
  if (value.approvalRequest !== undefined) {
    ...
    if (value.status !== "awaiting_approval") {
      errors.push("approvalRequest is only valid when task.status is awaiting_approval");
    }
...
function defaultCapabilityScope(id) {
  return id === CAPABILITY_IDS.taskReply ? "task" : "device";
}

function defaultCapabilities(role = "agent") {
  const ids = role === "hub"
    ? [CAPABILITY_IDS.syncSnapshot, CAPABILITY_IDS.deviceHealth]
    : [CAPABILITY_IDS.taskCapture, CAPABILITY_IDS.taskReply, CAPABILITY_IDS.deviceHealth];
