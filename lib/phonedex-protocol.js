...
const MAX = Object.freeze({
  ...
  title: 240,
  text: 10000,
  transcriptEntry: 6000,
  transcriptEntries: 32,
  transcriptTotal: 30000,
  machineName: 160,
  workspaceName: 240,
  version: 64,
  message: 1000,
...
function addTaskProtocolFields(task, now = new Date().toISOString()) {
  ...
  const record = {
    ...
    ...(normalizeTaskTranscript(task.transcript).length > 0
      ? { transcript: normalizeTaskTranscript(task.transcript) }
      : {}),
    ...(question ? { question } : {}),
    ...(evidence ? { evidence } : {}),
...
function validateTask(value, errors) {
  ...
  if (value.transcript !== undefined) validateTaskTranscript(value.transcript, errors);
...
function normalizeTaskTranscript(value) {
  ...
  for (const candidate of value) {
    ...
    const text = typeof candidate.text === "string"
      ? candidate.text.trim().slice(0, MAX.transcriptEntry)
      : "";
    if (!text || total + text.length > MAX.transcriptTotal) continue;
    const role = ["user", "assistant", "system"].includes(candidate.role)
      ? candidate.role
      : "assistant";
    const createdAt = validTimestamp(candidate.createdAt) ? candidate.createdAt : "";
    const id = optionalBoundedString(candidate.id, MAX.id) || `message_${entries.length + 1}`;
    ...
    entries.push({
      ...
      text,
      ...(createdAt ? { createdAt } : {}),
      ...(candidate.source ? { source: boundedString(candidate.source, MAX.captureSource) } : {})
    });
    total += text.length;
    if (entries.length >= MAX.transcriptEntries) break;
  }
  return entries;
}

function validateTaskTranscript(value, errors) {
  if (!Array.isArray(value)) {
    errors.push("transcript must be an array");
    return;
  }
  if (value.length > MAX.transcriptEntries) {
    errors.push(`transcript must contain at most ${MAX.transcriptEntries} entries`);
  }
  let total = 0;
  value.forEach((entry, index) => {
    ...
    requiredStringValue(entry.id, `transcript[${index}].id`, MAX.id, errors);
    if (!["user", "assistant", "system"].includes(entry.role)) {
      errors.push(`transcript[${index}].role must be user, assistant, or system`);
    }
    requiredStringValue(entry.text, `transcript[${index}].text`, MAX.transcriptEntry, errors);
    optionalTimestamp(entry.createdAt, `transcript[${index}].createdAt`, errors);
    optionalStringValue(entry.source, `transcript[${index}].source`, MAX.captureSource, errors);
    total += typeof entry.text === "string" ? entry.text.length : 0;
  });
  if (total > MAX.transcriptTotal) {
    errors.push(`transcript exceeds ${MAX.transcriptTotal} characters`);
  }
}
